				...
				return TypeGuard.TUnknown(left)
					...
					: TypeGuard.TAny(left)
						...
						: TypeGuard.TNever(left) ||
							  ...
							  (TypeGuard.TLiteralBoolean(left) &&
						...
						: TypeGuard.TUnion(type)
							? (() => {
									const narrowed = type.anyOf.filter(
										(inner) =>
											TypeExtends.Extends(inner, union) !==
											TypeExtendsResult.False,
									);
									return narrowed.length === 1
										? TypeClone.Type(narrowed[0], options)
...
var exportFiles = async ({ bundles, messages, variants, settings }) => {
	...
	for (const locale in files) {
		const sortDirection =
			settings?.["plugin.inlang.messageFormat"]?.sort ?? void 0;
